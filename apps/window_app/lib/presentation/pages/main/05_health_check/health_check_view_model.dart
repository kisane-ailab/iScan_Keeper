...
class HealthCheckViewModel extends _$HealthCheckViewModel {
  ...
  HealthCheckState build() {
    ...
    // health_check만 필터링 + mute rule에 매칭되는 로그 제외
    final healthCheckLogs = allLogs.where((entity) {
      if (!entity.isHealthCheck) return false;
      // mute rule에 매칭되면 제외
    ...
    // 환경별 로그 분리
    final prodLogs = healthCheckLogs.where((e) => e.isProduction).toList();
    final devLogs = healthCheckLogs.where((e) => e.isDevelopment).toList();
